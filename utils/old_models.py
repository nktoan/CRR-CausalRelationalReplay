...
def get_parser() -> ArgumentParser:
    ...
    parser.add_argument('--temp', type=float, default = 2.75,
...
class CRRPP(ModifiedContinualModel):
    """
    ...
    def __init__(self,  backbone, loss, args, transform , projection : bool = True, feat_dim_projection: int = 128, feat_dim_relation: int = 16, aggregation: str = 'cat') -> None:
        ...
        Return:
            None
        """
        super(CRRPP, self).__init__(backbone, loss, args, transform)
        ...
        self.scheduler = MultiStepLR(self.opt, milestones=[t for sub_list in [[i + 20, i + 40] for i in range(0, 250, 50)] for t in sub_list], gamma=0.8) #verbose = True)
    ...
    def save_buffer(self, t_idx: int, path: str = f'/content/drive/MyDrive/TOAN/thesis_weight/cifar10/') -> None:
        buf_x, buf_y = self.buffer.get_all_data()
