...
def print_mean_accuracy(mean_acc: np.ndarray, task_number: int,
    ...
    if setting == 'domain-il':
        ...
        mean_acc_class_il, mean_acc_task_il, mean_acc_class_il_pred, mean_acc_task_il_pred  = mean_acc
        print('\nAccuracy for {} task(s): \t [Class-IL-NCM]: {} %'
              ' \t [Task-IL-NCM]: {} %  \t [Class-IL-RR]: {} %  \t [Task-IL-RR]: {} % \n'.format(task_number, round(
            mean_acc_class_il, 2), round(mean_acc_task_il, 2), round(mean_acc_class_il_pred, 2), round(mean_acc_task_il_pred, 2), file=sys.stderr))
